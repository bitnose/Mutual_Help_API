...
// 1
final class CategoryDemandPivot: PostgreSQLUUIDPivot {
    // 2
    var id: UUID?
    // 3
    ...
    typealias Right = Demand
    // 5
    static let leftIDKey: LeftIDKey = \.categoryID
    static let rightIDKey: RightIDKey = \.demandID
    
    // 6
    init(_ categoryID: Category, _ demandID: Demand) throws {
        self.categoryID = try categoryID.requireID()
        self.demandID = try demandID.requireID()
    }
}

extension CategoryDemandPivot: ModifiablePivot {} // 9
...
/*
 ...
 3. Create the table for CategoryDemandPivot in the database.
 4. Use addProperties(to:) to add all the fields to the database.
 5. Add a reference between the demandID property on CategoryDemandPivot and the id property on Demand. This sets up the foreign key constraint. .cascade sets a cascade schema reference action when you delete the demand. This means that the relationship is automatically removed instead of an error being thrown.
 6. Add a reference between the categoryID property on CategoryDemandPivot and the id property on Category. This sets up the foreign key constraint. Also set the schema reference action for deletion when deleting the category.
 
 */

extension CategoryDemandPivot: Migration {// 1
    ...
    static func prepare(on connection: PostgreSQLConnection) -> Future<Void> { // 2
        return Database.create(self, on: connection) { builder in // 3
            ...
            builder.reference(from: \.demandID, to: \Demand.id, onDelete: .cascade) // 5
