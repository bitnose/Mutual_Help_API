...
/// - departmentID : ID of the Department (parent)
...
//    }
    ...
    static func updateCity(_ req: Request, data: EditCityData) throws -> Future<Void> {
        
        return City.query(on: req).filter(\City.id == data.cityID).first().flatMap(to: Void.self) { foundCity in
            guard let existingCity = foundCity else {throw Abort(.notFound)}
            existingCity.city = data.newName
            existingCity.departmentID = data.departmentID
            return existingCity.save(on: req).transform(to: ())
        }
    }
