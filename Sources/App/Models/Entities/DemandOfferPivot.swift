...
// 1
final class DemandOfferPivot: PostgreSQLUUIDPivot {
    // 2
    var id: UUID?
    // 3
    ...
    // 4
    typealias Left = Demand
    typealias Right = Offer
    // 5
    static let leftIDKey: LeftIDKey = \.demandID
    static let rightIDKey: RightIDKey = \.offerID
    
    // 6
    init(_ demandID: Demand, _ offerID: Offer) throws {
        self.demandID = try demandID.requireID()
        self.offerID = try offerID.requireID()
    }
}

extension DemandOfferPivot: ModifiablePivot {} // 9
...
extension DemandOfferPivot: Migration {// 1
