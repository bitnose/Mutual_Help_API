...
// 1
final class CategoryOfferPivot: PostgreSQLUUIDPivot {
    // 2
    var id: UUID?
    // 3
    ...
    // 6
    init(_ categoryID: Category, _ offerID: Offer) throws {
        self.categoryID = try categoryID.requireID()
        self.offerID = try offerID.requireID()
    }
}

extension CategoryOfferPivot: ModifiablePivot {} // 9
...
/*
 ...
 3. Create the table for OfferCategoryPivot in the database.
 4. Use addProperties(to:) to add all the fields to the database.
 5. Add a reference between the offerID property on OfferCategoryPivot and the id property on Offer. This sets up the foreign key constraint. .cascade sets a cascade schema reference action when you delete the offer. This means that the relationship is automatically removed instead of an error being thrown.
 6. Add a reference between the categoryID property on OfferCategoryPivot and the id property on Category. This sets up the foreign key constraint. Also set the schema reference action for deletion when deleting the category.
 
 */

extension CategoryOfferPivot: Migration {// 1
    ...
    static func prepare(on connection: PostgreSQLConnection) -> Future<Void> { // 2
        return Database.create(self, on: connection) { builder in // 3
            ...
            builder.reference(from: \.offerID, to: \Offer.id, onDelete: .cascade) // 5
